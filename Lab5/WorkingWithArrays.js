...
export default function WorkingWithArrays(app) {
  ...
  app.delete("/lab5/todos/:id", (req, res) => {
    ...
    const todoIndex = todos.findIndex((t) => t.id === parseInt(id));
    if (todoIndex === -1) {
        res.status(400).json({message: `Unable to delete Todo with ID ${id}`});
        return;
    }
    todos.splice(todoIndex, 1);
    res.sendStatus(200);
  ...
  app.put("/lab5/todos/:id", (req, res) => {
    ...
    res.sendStatus(200);
  });//this new implementation does not respond with the todos array, 
  //but responds with a simple OK status code of 200. 
